...
class User extends UFModel {
	...
	/*
		restituisce un vettore di tutti gli studi effettuati dall'esperto
	*/
	public function getStudiEsperto() {

		$studi = $this->hasMany('UserFrosting\Studio');
		return $studi->getResults();
    }

	public function getStudiUtente() {
		$link_table = Database::getSchemaTable('utAss_as_studio_user')->name;


		$studi = $this->belongsToMany('UserFrosting\Studio', $link_table);
		return $studi->getResults();
    }
