...
namespace UserFrosting;
   ...
   $app->get('/valutatore/?', function () use ($app) {   
		$controller = new UF\GroupController($app);
		return $controller->pageDashboard(); 
    });

	$app->get('/valutatore/definiscistudio/?', function () use ($app) {    
		$controller = new UF\GroupController($app);
		return $controller->pageDefinisciStudio();                 
    });

	$app->post('/valutatore/definiscistudio/?', function () use ($app) {    
		$controller = new UF\GroupController($app);
		return $controller->updateByFormStudio();                 
    });


    $app->get('/valutatore/analisistudio/?', function () use ($app) {    
		$controller = new UF\GroupController($app);
		return $controller->pageAnalisiStudio();     
    });

	$app->get('/valutatore/associa/?', function () use ($app) {    
		$controller = new UF\GroupController($app);
		return $controller->pageAssocia();                 
        
    });  

	    $app->get('/utente/?', function () use ($app) {    
		$controller = new UF\GroupController($app);
		return $controller->pageUtente();                 

    });
	    
    	$app->post('/utente/?', function () use ($app) {
    		$controller = new UF\GroupController($app);
    		return $controller->iniziaTask();
    	
    	});
	    		 

	$app->get('/utente/task/?', function () use ($app) { 
		$controller = new UF\GroupController($app);
		return $controller->pageTask();                 
